...
// implementation -----------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
const getI18nSvelteStore = (baseLocale = '') => {
    const currentLocale = writable(baseLocale);
    const isLoading = writable(false);
    let i18nObjectInstance = getFallbackProxy();
    const i18nObjectInstanceStore = writable(i18nObjectInstance);
    let getTranslationForLocale;
    let initFormatters;
    const initI18n = async (newLocale, getTranslationForLocaleCallback, initFormattersCallback) => {
        getTranslationForLocale = getTranslationForLocaleCallback;
        initFormatters = initFormattersCallback || (() => ({}));
        await setLocale(newLocale);
    };
    const setLocale = async (newLocale) => {
        if (!newLocale || !getTranslationForLocale)
            return;
        isLoading.set(true);
        const translation = await getTranslationForLocale(newLocale);
    ...
    const locale = derived(currentLocale, (newLocale, set) => set(newLocale));
    const isLoadingLocale = derived(isLoading, (loading, set) => set(loading));
    const LL = new Proxy({}, {
        get: (_target, key) => key === 'subscribe' ? i18nObjectInstanceStore.subscribe : i18nObjectInstance[key],
    });
    return {
        initI18n,
        setLocale,
...
export { LL, LL as default, initI18n, isLoadingLocale, locale, setLocale };
//# sourceMappingURL=svelte-store.mjs.map
