...
const getI18nSvelteStore = (baseLocale = '') => {
    const currentLocale = store.writable(baseLocale);
    const isLoading = store.writable(false);
    let i18nObjectInstance = getFallbackProxy();
    const i18nObjectInstanceStore = store.writable(i18nObjectInstance);
    ...
    const locale = store.derived(currentLocale, (newLocale, set) => set(newLocale));
    const isLoadingLocale = store.derived(isLoading, (loading, set) => set(loading));
    const LL = new Proxy({}, {
...
exports.isLoadingLocale = isLoadingLocale;
exports.locale = locale;
exports.setLocale = setLocale;
//# sourceMappingURL=svelte-store.cjs.map
