...
// implementation -----------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
const initI18nReact = (baseLocale = '', getTranslationForLocale = () => ({}), initFormatters = () => ({})) => {
    const context = getI18nContext();
    const component = (props) => {
        const [isLoadingLocale, setIsLoadingLocale] = React__default["default"].useState(false);
        const [currentLocale, setCurrentLocale] = React__default["default"].useState(null);
        const [LL, setLL] = React__default["default"].useState(getFallbackProxy());
        const setLocale = async (newLocale) => {
            setIsLoadingLocale(true);
            const translation = getTranslationForLocale(newLocale);
            const formatters = initFormatters(newLocale);
            setLL(i18nObject(newLocale, translation instanceof Promise ? await translation : translation, formatters instanceof Promise ? await formatters : formatters));
        ...
        return React__default["default"].createElement(context.Provider, { value: ctx }, props.children);
    };
    return { component, context };
};
const getI18nContext = () => React__default["default"].createContext({});

exports.initI18nReact = initI18nReact;
//# sourceMappingURL=react-context.cjs.map
