...
const logger$2 = createLogger(console);
...
const createPath = async (path) => {
    try {
        ...
        logger$2.error(`createPath: ${path}`, e);
        return false;
    }
};
...
const deleteFolderRecursive = async (path) => {
    try {
        ...
        logger$2.error(`deleteFolderRecursive: ${path}`, e);
        return false;
    }
};
...
const writeNewFile = async (path, file, content) => {
    ...
    logger$2.info(`generated file: ${file}`);
};
...
const isEsm = ((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('webpack-plugin-typesafe-i18n.js', document.baseURI).href)) || '').endsWith('.mjs');
const importFile = async (file, outputError = true) => {
    ...
    return (_a = (await (function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(importPath).catch((e) => {
        outputError && logger$2.error(`import failed for ${importPath}`, e);
        return null;
...
const generate = async (translations, config = {}, version, logger = logger$2, forceOverride = false) => {
...
const parseLanguageFile = async (outputPath, locale, tempPath) => {
    ...
    if (!(await doesPathExist(originalPath))) {
        logger$2.info(`could not load base locale file '${locale}'`);
        return null;
    }
    ...
    if (!languageImport) {
        logger$2.error(`could not read default export from language file '${locale}'`);
        return null;
    }
...
const parseAndGenerate = async (config, version) => {
    if (first) {
        ...
        logger$1.info('files were modified => looking for changes ...');
    }
    ...
    const languageFile = (locale && (await parseLanguageFile(outputPath, locale, path.resolve(tempPath, `${debounceCounter}`)))) || {};
    const translations = getDefaultExport(languageFile);
    await generate(translations, { ...config, baseLocale: locale, locales }, version, logger$1, firstLaunchOfGenerator);
    ...
    logger$1.info('... all files are up to date');
};
...
const startGenerator = async (config, watchFiles = true) => {
    logger$1 = createLogger(console, !watchFiles);
    ...
    const version = parseTypescriptVersion(ts__default["default"].versionMajorMinor);
    ...
    watchFiles && chokidar.watch(outputPath).on('all', () => debounce(onChange));
    logger$1.info(`generating files for ${shouldGenerateJsDoc ? 'JavaScript with JSDoc notation' : `TypeScript version: '${ts__default["default"].versionMajorMinor}.x'`}`);
    logger$1.info(`options:`, parsedConfig);
    watchFiles && logger$1.info(`watcher started in: '${outputPath}'`);
    if (!watchFiles) {
        await onChange();
        logger$1.info(`generating files completed`);
    }
};

const logger = createLogger(console, true);
let started = false;
class TypesafeI18nWebpackPlugin {
    constructor(config) {
        if (config) {
...
exports["default"] = TypesafeI18nWebpackPlugin;
//# sourceMappingURL=webpack-plugin-typesafe-i18n.js.map
